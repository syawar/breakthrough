...
//import boardgame.TreeNode;
import boardgame.Board;
import boardgame.Move;
import boardgame.Player;

public class SaqFinal extends Player {
    Random rand = new Random();
    public int maxRank=0;
    /** Provide a default public constructor */
    public SaqFinal() { super("syawar"); }
     ...
     public Move chooseMove(Board theboard) {
        ...
        pieces=this.Position(board,myColor);
		  ...
		  int[][] oppieces = new int[2*BTBoard.SIZE][2];
    		...
    		for( int i = 0; i < 2*BTBoard.SIZE; i++ ) {
	    			int orig1 = pieces[i][0];
	    			int orig2 = pieces[i][1];
	    			int counter=0;
	    			...
	    			for( int d = -1; d < 2; d++ ) {
						...
						if( board.getPieceAt( dest1,dest2)!=myColor )
		   			 {
